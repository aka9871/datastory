...
import { useListClientDashboards, useGetClient, useCreateDashboard, useUpdateDashboard, useDeleteDashboard, getListClientDashboardsQueryKey } from "@workspace/api-client-react";
...
import { Pencil, Trash2, Plus, Loader2, ArrowLeft, ExternalLink } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { Badge } from "@/components/ui/badge";

export default function AdminClientDashboards() {
  const [, params] = useRoute("/admin/clients/:id/dashboards");
  const clientId = params?.id ? parseInt(params.id) : null;

  const { data: client, isLoading: isLoadingClient } = useGetClient(clientId as number, {
    query: { enabled: !!clientId }
  });
  const { data: dashboards, isLoading: isLoadingDashboards } = useListClientDashboards(clientId as number, {
    query: { enabled: !!clientId }
  });
  const isLoading = isLoadingClient || isLoadingDashboards;
  ...
  const onSave = async () => {
    if (!formData.title || !formData.lookerstudioUrl || !clientId) {
      toast({ title: "Validation Error", description: "Title and Looker Studio URL are required", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: getListClientDashboardsQueryKey(clientId) });
    } catch (error: any) {
  ...
  const onDelete = async () => {
    if (!dashboardToDelete || !clientId) return;
    try {
  ...
  return (
    <AdminLayout>
      <div className="mb-6">
        <Link href="/admin/clients" className="inline-flex items-center text-sm font-medium text-muted-foreground hover:text-foreground mb-4">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-serif font-bold tracking-tight mb-2">
            {client ? client.name : "Client"} Dashboards
          </h1>
          <p className="text-muted-foreground">Manage reporting dashboards for this client.</p>
        </div>
        <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
          <DialogTrigger asChild>
            <Button onClick={handleCreateOpen} disabled={!client}>
          ...
          <DialogContent className="sm:max-w-[500px] rounded-none max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>Add a new dashboard for {client?.name}.</DialogDescription>
            </DialogHeader>
        ...
        <Dialog open={!!editingDashboard} onOpenChange={(open) => !open && setEditingDashboard(null)}>
          <DialogContent className="sm:max-w-[500px] rounded-none max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>Update dashboard settings for {client?.name}.</DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow>
                <TableCell colSpan={5} className="h-24 text-center text-muted-foreground">
                  No dashboards found for this client. Create one to get started.
                </TableCell>
              </TableRow>
            ) : (
              dashboards?.sort((a, b) => a.order - b.order).map((dashboard) => (
                <TableRow key={dashboard.id} className="border-border">
                  ...
                  <TableCell>{dashboard.order}</TableCell>
                  <TableCell>
                    {dashboard.active ? (
